...
/**
 * function is called only when filter parameters are used
 * reassigns new values to loadSpeechNet() to build another graph
 * @author Rodiana Koukouzeli
 */
function updateGraphThree(){
    var limit = $('#limit').val();
    ...
    $("#my_datavizthree").empty();
    $("#protocolGraphThree").empty();

    loadSpeechNet(limit,startDateAsLong,endDateAsLong);
}

/**
 * function is called when the user wants a sample of WP19 or WP20
 * @param limit
 * @param startDate
 * @param endDate
 * @author Rodiana Koukouzeli
 */
function getSpecificThree(limit, startDate, endDate){
    $("#my_datavizthree").empty();
    $("#protocolGraphThree").empty();
    loadSpeechNet(limit, startDate,endDate);
}


/**
 * builds new graph that represents only one specific parliament protocol,
 * only called after second submit button is clicked
 * @author Rodiana Koukouzeli
 */
function getProtocolSpeech(){
    alert("This may take a little while.");
    var protocol = document.getElementById("protocol").value;
    console.log(protocol);
    $("#my_datavizthree").empty();
    $("#protocolGraphThree").empty();
    loadProtocolSpeechNet(protocol);
}
 ...
 * @author Rodiana Koukouzeli
 */
function loadSpeechNet(limit, startDate, endDate){
...
// append the svg object to the body of the page
    const svg = d3.select("#my_datavizthree")
        .append("svg")
    ...
    $.ajax({
        url: "http://localhost:4567/network/speeches?limit=" + limit + "&startDate=" + startDate + "&endDate=" + endDate,
        type: 'GET',
        dataType: 'json',
        async: true,
        success: function (data) {
            ...
            const simulation = d3.forceSimulation(data.nodes)                 // Force algorithm is applied to data.nodes
                ...
                .force("collision", d3.forceCollide().radius(function (d){changeForceRadius(d.party);}))    // This force attracts nodes to the center of the svg area
                .on("end", ticked);
 ...
 * @author Rodiana Koukouzeli
 */
function loadProtocolSpeechNet(protocol){
    const margin = {top: 10, right: 30, bottom: 30, left: 40},
        width = 1500 - margin.left - margin.right,
        height = 1500 - margin.top - margin.bottom;

// append the svg object to the body of the page
    const svg = d3.select("#protocolGraphThree")
        .append("svg")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
        .append("g")
    ...
    $.ajax({
        url: "http://localhost:4567/network/speeches/protocol?protocol=" + protocol,
        type: 'GET',
        dataType: 'json',
        async: true,
        success: function (data) {
